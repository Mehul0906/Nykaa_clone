...
const Description = () => {
  ...
  const addToCart = () => {
    axios
      .post("http://localhost:3000/ProductData", {
        productId: id,
        quantity: 1, 
        image: data.image_url && data.image_url[0],
        title: data.name,
        price: data.price
      })
      .then((res) => {
  ...
  return (
    <>
      ...
      <div className="row">
        <div className="col-12 d-flex justify-content-betw  een flex-column flex-lg-row">
          <div className="col-12 col-lg-6">
              <img src={data.image_url} alt="" className="img-fluid" style={{width:"500px",height:"500px",margin:"auto"}} />
          </div>
          ...
          <div className="col-12 col-lg-6 mt-5 p-2 ">
            ...
            <Link to={"/Cart"}> <button
              onClick={addToCart}
              style={{ padding: "10px 20px", outline: "none", border: "none", backgroundColor: "rgb(232, 0, 113)", color: "white", marginBottom: "30px" }}
            >
              ADD TO BAG
            </button></Link>
